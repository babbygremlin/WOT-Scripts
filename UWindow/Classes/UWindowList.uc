...
var int			FastShownCount;
var bool		bInternalShown;
...
function UWindowList CreateItem(Class<UWindowList> C)
{
	...
	NewElement = New C;
	NewElement.bInternalShown = True;
...
function SetInternalShown(bool bShowItem)
{
	if(Sentinel != None)
	{
		if(!bInternalShown && bShowItem)
		{
			Sentinel.FastShownCount++;
			Sentinel.bItemOrderChanged = True;
		}
		
		if(!bShowItem && bInternalShown)
		{
			Sentinel.FastShownCount--;
			Sentinel.bItemOrderChanged = True;
		}
...
function UWindowList CopyExistingListItem(Class<UWindowList> ItemClass, UWindowList SourceItem)
{
	...
	I = Append(ItemClass);
	if(!SourceItem.bInternalShown)
		SetInternalShown(False);
	Sentinel.bItemOrderChanged = True;

	return I;
}

// for Listboxes and grids only (so far)
function bool ShowThisItem()
{
	return bInternalShown;
}
...
// ONLY valid if this doesn't override ShowThisItem and calls SetInternalShown instead.
function int FastCountShown() 
{
	return FastShownCount;
}
