...
function WindowEvent(WinMessage Msg, Canvas C, float X, float Y, int Key) 
{
	...
	case WM_LMouseDown:
		...
		{
			if(!MessageClients(Msg, C, X, Y, Key)) LMouseDown(X, Y);
		}
		break;	
	case WM_LMouseUp:
		if(!Root.CheckCaptureMouseUp())
		{
			if(!MessageClients(Msg, C, X, Y, Key)) LMouseUp(X, Y);
		}
		break;	
	case WM_RMouseDown:
	...
	case WM_MMouseUp:
		...
		break;	
	case WM_KeyDown:
		if(!PropagateKey(Msg, C, X, Y, Key)) KeyDown(Key, X, Y);
		break;	
	case WM_KeyUp:
		if(!PropagateKey(Msg, C, X, Y, Key)) KeyUp(Key, X, Y);
		break;	
	case WM_KeyType:
		if(!PropagateKey(Msg, C, X, Y, Key)) KeyType(Key, X, Y);
		break;	
	default:
		break;
	}
...
{
	local int i;
	local string ObjectClass, PropertyName, PropertyValue;
