...
// Flags.
//#if 1 //NEW
var(Advanced) bool  bStatic;       // Does not move or change over time.
//#else
...
//#if 1 //NEW
var bool				  bAnimLoopFinished; // Used in engine with FinishAnim(false) to signal end of a loop without stopping it.
//#endif
...
//#if 1 //NEW
var() const bool          bDeleteMe;     // Expose variable to UnrealEd so LDs can get around non-deleteable actor bug.
//#else
...
//#if 1 //NEW
var(Advanced) byte        DetailLevel;   // Actor detail level (0:Low - 4:High)
...
//#if 1 //NEW
var			  bool		  bCanFallOutOfWorld; // suppress fell out of world message
//#endif
...
//#if 1 //NEW
var bool bSpecialRotationRep;            // Hack to override Rotation replication statement for ParticleSprayers.
//#endif
...
//#if 1 //NEW
var(Movement) bool        bAllowClipping;    // If set to true, Actors with PHYS_Projectile will not be destroyed if they go outside the level.
//#endif
...
//#if 1 //NEW
var() class<Debug> DebugClass;						// class type of the debug object
var transient Debug DebugInterface;					// the debug object for this instance do not use this directly!
//#endif
...
simulated function sound GetDeploySound();							// Sound when player adds resource to citadel interface
simulated function sound GetRemoveSound();							// Sound when player removes resource to citadel interface
simulated function BeginEditingResource( int PlacedByTeam );			// Called for existing resources when the editor is started, and for new resources
simulated function EndEditingResource();								// Called only when the editor is ended - not called for temporary objects
simulated function BeginSelectResource();
simulated function EndSelectResource();
simulated function Actor GetBaseResource() { return None; }			// Return the parent trap component (e.g., WallSlabs return their Owner)
function bool SameLogicalActor( Actor Other ) { return Self == Other; } // items are always part of themselves
static function AdjustSpawnLocation( actor Other, out vector Loc, out rotator Rot );
simulated function bool IsResourceActive() { return false; }
simulated function bool ActivateResource( bool ChangeState ) { return false; }
simulated function bool DeactivateResource( bool ChangeState ) { return false; }
simulated function bool DeployResource( vector Loc, vector Normal );	// Places resource in level and adds parts for multiple part resources, called when the cursor is moved or when the player left clicks
simulated function bool RemoveResource();								// Removes resource from level - does not remove parts for multiple part resources
function Hide() { bHidden = true; }
function Show() { bHidden = false; }
...
//#if 1 //NEW
	unreliable if( Role==ROLE_Authority )
		Role, RemoteRole;
	unreliable if( Role==ROLE_Authority && ( Owner==None || Owner.bNetRelevant ) ) //ARL/MWP -- remove when Tim fixes Actor variable replication code.
		Owner;
//#else
//	unreliable if( Role==ROLE_Authority )
...
//#endif
	...
	unreliable if( Role==ROLE_Authority )
		AmbientSound;
	unreliable if( AmbientSound!=None && Role==ROLE_Authority )
...
//#endif
	...
	unreliable if( (bCollideActors || bCollideWorld) && Role==ROLE_Authority )
		bBlockActors, bBlockPlayers, CollisionRadius, CollisionHeight;

	// Location.
	unreliable if( !bCarriedItem && (bNetInitial || bSimulatedPawn || RemoteRole<ROLE_SimulatedProxy) && Role==ROLE_Authority )
		Location;
//#if 1 //NEW
	unreliable if( !bCarriedItem && ((DrawType==DT_Mesh) || (DrawType==DT_Brush) || (IsA('ParticleSprayer'))) && (bNetInitial || bSimulatedPawn || RemoteRole<ROLE_SimulatedProxy) && (Role==ROLE_Authority) )
//#else
...
//#if 1 //NEW
	unreliable if( Role==ROLE_Authority )
		VisibilityRadius, VisibilityHeight, DetailLevel, GoreLevel, SpriteProjForward, bMustFace, RenderIteratorClass;
//#endif
...
//#if 1 //NEW
native(261) final latent function FinishAnim( optional bool bKillLoop );
//#else
//native(261) final latent function FinishAnim( );
//#endif
native(263) final function bool HasAnim( name Sequence );
//#if 1 //NEW
intrinsic	   final function int GetAnimFrames( name Sequence );
//#endif
...
//#if 1 //NEW: Moved from Pawn.uc to accomodate Mashadar.
...
//#if 1 //NEW
//
// Similar to trace, but done on this actor only.
// Used to find if the given line intersects with this Actor, and if so, where.
//
native(1061) final function bool LineCheck
(
	out vector		HitLocation,
	out vector		HitNormal,
	vector			TraceStart,		// note: reverse order from Trace.
	optional vector	TraceEnd,		// defaults to Actor's location.
	optional vector TraceExtent		// defaults to vect(0,0,0)
);
//#endif
...
native final function PlayOwnedSound
(
	sound				Sound,
	optional ESoundSlot Slot,
	optional float		Volume,
...
event BroadcastMessage( coerce string Msg, optional bool bBeep, optional name Type )
{
	...
	if ( Level.Game.AllowsBroadcast(self, Len(Msg)) )
		for( P=Level.PawnList; P!=None; P=P.nextPawn )
			if( P.bIsPlayer || P.IsA('DemoRecSpectator') )
...
simulated function DM( coerce string Msg )
{
	local Pawn P;
