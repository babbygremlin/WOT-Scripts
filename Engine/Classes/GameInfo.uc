...
var	  bool					  bOverTime;
//#if 1 //NEW: Changes from Jack Porter (1999.10.19)
...
{
	StartTime = 0;
//#if 0 //NEW: Changes from Jack Porter (1999.10.19)
//	SetTimer(1.0, true);
//#endif
...
//#if 1 //NEW
	if( Level.NetMode == NM_Standalone && ( Other.DetailLevel > GetMaxDetailLevel() || Other.GoreLevel > GetGoreDetailLevel() ) )
	{
		// make sure the actor can be deleted, then, signal that it should be...
		Other.bStatic = false;
		return false;
	}
//#endif
...
//#if 1 //NEW
	// Allow players into a maxed out server if they have the GamePassword or AdminPassword.
	if
	(	(Level.NetMode != NM_Standalone) && AtCapacity(Options)
	&&	(GamePassword=="" || caps(InPassword)!=caps(GamePassword))		// cannot join if there is no gamepassword to use as an override.
	&&	(AdminPassword=="" || caps(InPassword)!=caps(AdminPassword))	// cannot join if there is no adminpassword to use as an override.
	)
//#else
...
//			Error = NeedPassword;
//#endif
			FailCode = "WRONGPW"; //NEW "NEEDPW";  -- NEEDPW results in an endless loop due to the saved password from WOT.ini
		}
		else
		{
//#if 0 //NEW
...
//#if 1 //NEW
	...
	&&	(GamePassword=="" || caps(InPassword)!=caps(GamePassword))		// override capacity via gamepassword if exists.
	&&	(AdminPassword=="" || caps(InPassword)!=caps(AdminPassword))	// override capacity via adminpassword if exists.
	)
	{
		if ( ClassIsChildOf(SpawnClass, class'Spectator') )
		{
			if ( (NumSpectators >= MaxSpectators)
				&& ((Level.NetMode != NM_ListenServer) || (NumPlayers > 0)) )	//NEW: Changes from Jack Porter (1999.10.19)
			{
				Error=MaxedOutMessage;
				return None;
			}
...
//#if 1 //NEW
//	if( Level.MP3Filename != "" )
//	{
//		NewPlayer.ConsoleCommand( "MP3 START "$ Level.MP3Filename );
//	}
//#endif
}
...
{
//#if 1 //NEW
	if( Other != None && Killer != None && Killer.Weapon != None )
	{
...
function DiscardInventory( Pawn Other )
{
	...
	{
//#if 1 //NEW
		dropped = Spawn(Other.DropWhenKilled,,Other.DropWhenKilled.Name,Other.Location);
//#else
