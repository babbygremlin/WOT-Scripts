...
{
	Target.Trigger( Other, OtherInstigator );
}

function DispatchUnTrigger( actor Target, actor Other, pawn OtherInstigator )
{
	Target.UnTrigger( Other, OtherInstigator );
}
//#endif
...
//#if 1 //NEW
				DispatchTrigger( A, Other, Other.Instigator );
//#else
						...
						Vector momentum, name damageType)
{
	...
	if ( bInitiallyActive && (TriggerType == TT_Shoot) && (Damage >= DamageThreshold) && (instigatedBy != None) )
	{
		...
		if( Event != '' )
			foreach AllActors( class 'Actor', A, Event )
//#if 1 //NEW
				DispatchTrigger( A, instigatedBy, instigatedBy );
//#else
...
function UnTouch( actor Other )
{
	...
	if( IsRelevant( Other ) )
	{
		...
		if( Event != '' )
			foreach AllActors( class 'Actor', A, Event )
//#if 1 //NEW
				DispatchUnTrigger( A, Other, Other.Instigator );
//#else
