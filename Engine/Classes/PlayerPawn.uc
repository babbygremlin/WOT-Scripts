...
replication
{
	...
	reliable if( bNetOwner && Role==ROLE_Authority )
		ViewTarget, ScoringType, HUDType, GameReplicationInfo, bFixedCamera, bNeverAutoSwitch;
	...
	reliable if( Role<ROLE_Authority )
		...
		SwitchLevel, SwitchCoopLevel, Kick, KickBan, KillAll, Summon, ActivateTranslator, Admin, AdminLogin, AdminLogout, Typing;
	unreliable if( Role<ROLE_Authority )
	...
	reliable if( Role==ROLE_Authority )
		ClientAdjustGlow, ClientChangeTeam, ClientTravel, ClientSetMusic, StartZoom, ToggleZoom, StopZoom, EndZoom, SetDesiredFOV, ClearProgressMessages, SetProgressColor, SetProgressMessage, SetProgressTime;
	unreliable if( Role==ROLE_Authority )
...
)
{
	...
	ViewRotation.Roll = 0;
//#if 1 //NEW
	Rot.Pitch = 0;	// Force player to always be straight up and down.
//#endif
	SetRotation(Rot);
			...
			if ( TotalTime > 0 )
				ForEach AllActors(class'Pawn', P)
					if ( (P != self) && (P.Velocity != vect(0,0,0)) && P.bBlockPlayers )
					{
						...
						AdjPCol = 0.00025 * PlayerReplicationInfo.Ping * ((P.Velocity - Velocity) Dot Normal(Dist));
						if ( VSize(Dist) < AdjPCol + CollisionRadius + CurrentMove.Delta * GroundSpeed * (Normal(Velocity) Dot Normal(Dist)) )
						{
							SavedPawn = P;
...
)
{
	...
	ForEach AllActors(class'Pawn', P)
		if ( (P != self) && (P.Velocity != vect(0,0,0)) && P.bBlockPlayers )
		{
			Dist = P.Location - Location;
			AdjPCol = 0.00025 * PlayerReplicationInfo.Ping * ((P.Velocity - Velocity) Dot Normal(Dist));
			if ( VSize(Dist) < AdjPCol + CollisionRadius + NewMove.Delta * GroundSpeed * (Normal(Velocity) Dot Normal(Dist)) )
			{
				SavedPawn = P;
				SavedRadius = P.CollisionRadius;
				Dist.Z = 0;
	...
	NetMoveDelta = 64.0/Player.CurrentNetSpeed;
...
{	
	...
	if ( ViewTarget != None )
	{
		ViewTarget.PlaySound(ASound, SLOT_None, 16.0, bInterrupt);
		ViewTarget.PlaySound(ASound, SLOT_Interface, 16.0, bInterrupt);
		ViewTarget.PlaySound(ASound, SLOT_Misc, 16.0, bInterrupt);
		ViewTarget.PlaySound(ASound, SLOT_Talk, 16.0, bInterrupt);
	}
	else
	{
		PlaySound(ASound, SLOT_None, 16.0, bInterrupt, 1000000);
		PlaySound(ASound, SLOT_Interface, 16.0, bInterrupt, 1000000);
		PlaySound(ASound, SLOT_Misc, 16.0, bInterrupt, 1000000);
		PlaySound(ASound, SLOT_Talk, 16.0, bInterrupt, 1000000);
	}
}
...
//#if 1 //NEW
// Can override to substitute given sequence if desired.
...
{
//#if 1 //NEW
	if ( GetAnimGroupSub(Sequence) == 'Gesture' ) 
//#else
	...
	if (Player.Console != None)
	{
		if(bActivate)
		{
			if(Player.Console.TimeDemo == None)
				Player.Console.TimeDemo = Spawn(Player.Console.TimeDemoClass);
...
{
	local float SmoothTime, FOVScale, MouseScale, AbsSmoothX, AbsSmoothY;
...
{
//#if 1 //NEW
	if ( !bAnimTransition && (GetAnimGroupSub(AnimSequence) != 'Gesture') )
//#else
...
//#endif
	{
		if ( bNotForward )
	 	{
//#if 1 //NEW
		 	 if ( GetAnimGroupSub(AnimSequence) != 'Waiting' )
//#else
...
//#if 1 //NEW
		else if ( GetAnimGroupSub(AnimSequence) == 'Waiting' )
//#else
...
ignores SeePlayer, HearNoise, Bump;
	...
	function AnimEnd()
	{
		...
		if (Physics == PHYS_Walking)
		{
			...
			{
//#if 1 //NEW
				MyAnimGroup = GetAnimGroupSub(AnimSequence);
//#else
...
//#endif
	...
	function ProcessMove(float DeltaTime, vector NewAccel, eDodgeDir DodgeMove, rotator DeltaRot)	
	{
		...
		if ( bPressedJump )
			DoJump();
//#if 1 //NEW
		if ( (Physics == PHYS_Walking) && (GetAnimGroupSub(AnimSequence) != 'Dodge') )
//#else
...
//#endif
		{
			...
			{
//#if 1 //NEW
				if ( (!bAnimTransition || (AnimFrame > 0)) && (GetAnimGroupSub(AnimSequence) != 'Landing') )
//#else
...
//#endif
				{
					...
					{
//#if 1 //NEW
						if ( (GetAnimGroupSub(AnimSequence) == 'Waiting') || (GetAnimGroupSub(AnimSequence) == 'Gesture') || (GetAnimGroupSub(AnimSequence) == 'TakeHit') )
//#else
...
//#if 1 //NEW
						&& (GetAnimGroupSub(AnimSequence) != 'Gesture') ) 
//#else
...
//#if 1 //NEW
		AnimGroupName = GetAnimGroupSub(AnimSequence);		
//#else
...
ignores SeePlayer, HearNoise, Bump;
	...
	function AnimEnd()
	{
		...
		{
//#if 1 //NEW
			if ( GetAnimGroupSub(AnimSequence) == 'TakeHit' )
//#else
