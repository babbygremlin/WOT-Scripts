//------------------------------------------------------------------------------
// DecreaseRareChargesLeech.uc
// $Author: Mfox $
// $Date: 1/05/00 2:27p $
// $Revision: 2 $
//
// Description:	Decreases the victim's Rare angreal charges over time.
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
class DecreaseRareChargesLeech expands Leech;
...
function AffectHost( optional int Iterations )
{
	...
	if( Owner != None && Pawn(Owner).Health > 0 )
	{
		...
		for( Inv = Pawn(Owner).Inventory;
			 ...
			 Inv = Inv.Inventory )
		{
			// If you find a ter'angreal and it is Rare.
			if( AngrealInventory(Inv) != None && AngrealInventory(Inv).bRare )
			{
